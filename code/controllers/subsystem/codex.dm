...
/datum/controller/subsystem/codex/proc/prepare_search_database(drop_existing = FALSE)
	if(Debugger?.enabled && !FORCE_CODEX_DATABASE)
		index_disabled = TRUE
		to_chat(world, span_interface("Codex: Debug server detected. DB operation disabled. See _compile_options.dm."))
		log_world("Codex: Codex DB generation Skipped")
		return
	#if FORCE_CODEX_DATABASE == TRUE
	to_chat(world, span_interface("Codex: Debug server detected. Override flag set, Dropping and regenerating index."))
	...
	if(drop_existing)
		to_chat(world, span_interface("Codex: Deleting old index..."))
		...
		to_chat(world, span_interface("Codex: Preparing Search Database"))
		log_world("Codex: Preparing Search Database")

	index_generating = TRUE
	if(!rustg_file_exists(CODEX_SEARCH_INDEX_FILE))
		if(!drop_existing)
			to_chat(world, span_interface("Codex: Database missing, building..."))
		else
			to_chat(world, span_interface("Codex: Building new database..."))
		create_db()
		build_db_index()
	...
	if(!cursor.Execute(codex_index))
		index_disabled = TRUE
		can_fire = FALSE
		to_chat(world, span_interface("Codex: ABORTING! Database error: [cursor.Error()] | [cursor.ErrorMsg()]"))
	...
	if(db_serial != GLOB.revdata.commit)
		if(db_serial == CODEX_SERIAL_FALLBACK)
			to_chat(world, span_interface("Codex: Fallback Database Serial detected. Data may be inaccurate or out of date."))
		else
			if(drop_existing)
				CRASH("Codex DB generation issue. Freshly generated index serial is bad. Is: [db_serial] | Expected: [GLOB.revdata.commit]")
			to_chat(world, span_interface("Codex: Database out of date, Rebuilding..."))
			prepare_search_database(TRUE) //recursiveness funny,,
			return

	if(!drop_existing)
		//Loading an old database, we need to flush the dynamic cache.
		to_chat(world, span_interface("Codex: Flushing Dynamic Index"))
	...
	if(drop_existing)
		to_chat(world, span_interface("Codex: Collation complete.\nCodex: Index ready."))
	else
		to_chat(world, span_interface("Codex: Database Serial validated.\nCodex: Loading complete."))

/datum/controller/subsystem/codex/proc/create_db()
	// No index? Make one.

	to_chat(world, span_interface("Codex: Writing new database file..."))
	//We explicitly store the DB in the root directory, so that TGS builds wipe it.
	codex_index = new(CODEX_SEARCH_INDEX_FILE)

	to_chat(world, span_interface("Codex: Writing Schema (1/3): Metadata"))
	...
	if(!init_cursor.Execute(codex_index))
		to_chat(world, span_interface("Codex: ABORTING! Database error: [init_cursor.Error()] | [init_cursor.ErrorMsg()]"))
		CRASH("Codex: ABORTING! Database error: [init_cursor.Error()] | [init_cursor.ErrorMsg()]")

	to_chat(world, span_interface("Codex: Writing Schema (2/3): Baked Index"))
	...
	to_chat(world, span_interface("Codex: Writing Schema (3/3): Dynamic Index"))
	...
	to_chat(world, span_interface("Codex: Schema complete, Writing serial..."))
...
/datum/controller/subsystem/codex/proc/build_db_index()
	to_chat(world, span_interface("Codex: Building search index."))

	var/database/query/cursor = new
	var/total_entries = length(all_entries)
	to_chat(world, span_interface("\tCodex: Collating [total_entries] records..."))
	...
	for(var/datum/codex_entry/entry as anything in all_entries)
		...
		if((!(record_id % 100)) || (record_id == total_entries))
			to_chat(world, span_interface("\tCodex: [record_id]/[total_entries]..."))
