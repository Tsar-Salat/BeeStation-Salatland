...
/**
 * Getter function for a mind's quirk
 *
 * Arguments:
 * * quirktype - the type of the quirk to acquire e.g. /datum/quirk/some_quirk
 *
 * Returns the mind's quirk datum if the mob this is called on has the quirk, null on failure
 */
/datum/mind/proc/get_quirk(quirktype)
