...
/datum/ai_behavior/fetch/perform(delta_time, datum/ai_controller/controller)
	...
	if(fetch_thing.anchored || !isturf(fetch_thing.loc) || isfood(fetch_thing) || !living_pawn.CanReach(fetch_thing))
...
/datum/ai_behavior/eat_snack/perform(delta_time, datum/ai_controller/controller)
	...
	if(!istype(snack) || !isfood(snack) || !(isturf(snack.loc) || ishuman(snack.loc)))
		finish_action(controller, FALSE)
