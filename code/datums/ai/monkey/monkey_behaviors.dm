...
/datum/ai_behavior/monkey_attack_mob/proc/monkey_attack(datum/ai_controller/controller, mob/living/target, delta_time, disarm)
	...
	// no de-aggro
	if(controller.blackboard[BB_MONKEY_AGGRESSIVE] && !(HAS_TRAIT(target, TRAIT_MONKEYFRIEND))) // so aggro can drop
		return
	...
	if(controller.blackboard[BB_MONKEY_ENEMIES][target_ref] <= 0 || (HAS_TRAIT(target, TRAIT_MONKEYFRIEND))) //so aggro can drop
