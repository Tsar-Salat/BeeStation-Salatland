...
/datum/component/style/proc/hotswap(mob/living/source, atom/target, obj/item/weapon, proximity_flag, click_parameters)
	...
	if((rank < hotswap_rank) || !isitem(target) || !(target in source.GetAllContents()))
		return
	...
	//var/datum/component/storage/concrete/atom_storage = item_target.loc.GetComponent(/datum/component/storage/concrete)

	if(!SEND_SIGNAL(src, COMSIG_TRY_STORAGE_CAN_INSERT, weapon, source, FALSE))
		source.balloon_alert(source, "unable to hotswap!")
		return

	SEND_SIGNAL(src, COMSIG_TRY_STORAGE_INSERT, weapon, source, override = TRUE)
...
/datum/component/style/proc/on_crusher_detonate(datum/source, mob/living/target, obj/item/kinetic_crusher/crusher, backstabbed)
	...
	add_action(ACTION_MARK_DETONATED, round((backstabbed ? 60 : 30) * (ismegafauna(target) ? 1.5 : 1)))
