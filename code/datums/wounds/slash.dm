...
/datum/wound/slash/handle_process(delta_time, times_fired)
	...
	if(blood_flow < minimum_flow)
		if(demotes_to)
			...
			to_chat(victim, "<span class='green'>The cut on your [limb.plaintext_zone] has [no_bleeding ? "healed up" : "stopped bleeding"]!</span>")
			qdel(src)
...
/datum/wound/slash/proc/tool_cauterize(obj/item/I, mob/user)
	...
	user.visible_message("<span class='green'>[user] cauterizes some of the [bleeding_wording] on [victim].</span>", "<span class='green'>You cauterize some of the [bleeding_wording] on [victim].</span>")
...
/datum/wound/slash/proc/suture(obj/item/stack/medical/suture/I, mob/user)
	...
	if(!do_after(user, base_treat_time * self_penalty_mult, target=victim, extra_checks = CALLBACK(src, .proc/still_exists)))
		return
	var/bleeding_wording = (no_bleeding ? "cuts" : "bleeding")
	user.visible_message("<span class='green'>[user] stitches up some of the [bleeding_wording] on [victim].</span>", "<span class='green'>You stitch up some of the [bleeding_wording] on [user == victim ? "yourself" : "[victim]"].</span>")
