...
/datum/symptom/toxoplasmosis/Activate(datum/disease/advance/A)
	...
	if(A.stage >= 4)
		...
		if(mania)
			var/obj/item/organ/ears/human/cat/ears = M.getorgan(/obj/item/organ/ears/human/cat)
			var/obj/item/organ/tail/tail = M.getorgan(/obj/item/organ/tail)
			if(tail && !istype(tail, /obj/item/organ/tail/human/cat))
				...
				if(dnacounter >= 5)
					if(!tail)
						var/obj/item/organ/tail/human/cat/cattail = new()
						...
						var/obj/item/organ/ears/human/cat/catears = new()
...
/datum/symptom/toxoplasmosis/proc/Pounce(mob/living/cat, mob/living/carbon/human/H)
	if(istype(cat, /mob/living/simple_animal/pet/cat))
		...
		if(ishuman(cat))
			...
			var/obj/item/organ/ears/human/cat/targetears = target.getorgan(/obj/item/organ/ears/human/cat)
			var/obj/item/organ/tail/human/cat/targettail = target.getorgan(/obj/item/organ/tail/human/cat)
			if(!H.getorgan(/obj/item/organ/tail/human/cat) && targettail)
				...
				if(targettail.organ_flags & ORGAN_FAILING)
					...
					H.emote("laugh")
			else if(!H.getorgan(/obj/item/organ/ears/human/cat) && targetears)
...
/datum/symptom/toxoplasmosis/proc/findcat(mob/living/carbon/human/M, requiresears = TRUE, requirestail = TRUE) //return a cat, or someone with cat ears and a tail
	for(var/mob/living/L in oviewers(7, M))
		...
		if(ishuman(L))
			...
			var/ears = H.getorgan(/obj/item/organ/ears/human/cat)
			var/tail = H.getorgan(/obj/item/organ/tail/human/cat)
