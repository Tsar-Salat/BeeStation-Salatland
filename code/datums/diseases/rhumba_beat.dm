	...
	switch(stage)
		...
		if(5)
			...
			if(DT_PROB(29, delta_time))
				explosion(affected_mob, devastation_range = -1, light_impact_range = 2, flame_range = 2, flash_range = 3, adminlog = FALSE, magic = TRUE, explosion_cause = src) // This is equivalent to a lvl 1 fireball
