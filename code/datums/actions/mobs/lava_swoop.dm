...
/datum/action/cooldown/mob_cooldown/lava_swoop/Grant(mob/living/M)
	. = ..()
	LAZYOR(M.weather_immunities, "lava")
	ADD_TRAIT(M, TRAIT_NOFIRE, src)

/datum/action/cooldown/mob_cooldown/lava_swoop/Remove(mob/living/M)
	. = ..()
	LAZYREMOVE(M.weather_immunities, "lava")
