...
/datum/quirk/bilingual/add(client/client_source)
	...
	if(!language_type || quirk_target.has_language(language_type))
	...
	if(isnull(speakable) || speakable)
		quirk_target.grant_language(language_type, SPOKEN_LANGUAGE|UNDERSTOOD_LANGUAGE, source = LANGUAGE_QUIRK)
	else if(language_skill == "100%")
		quirk_target.grant_language(language_type, UNDERSTOOD_LANGUAGE, source = LANGUAGE_QUIRK)
	else
		quirk_target.grant_partial_language(language_type, text2num(language_skill), source = LANGUAGE_QUIRK)

/datum/quirk/bilingual/remove()
	if(QDELING(quirk_target))
		return
	quirk_target.remove_all_languages(source = LANGUAGE_QUIRK)
	quirk_target.remove_all_partial_languages(source = LANGUAGE_QUIRK)
