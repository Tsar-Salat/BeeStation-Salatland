...
/datum/quirk/csl/add(client/client_source)
	if(iscarbon(quirk_target))
		quirk_target.remove_language(/datum/language/common, UNDERSTOOD_LANGUAGE, LANGUAGE_SPECIES)
	else
		quirk_target.remove_language(/datum/language/common, UNDERSTOOD_LANGUAGE, LANGUAGE_ATOM)
	quirk_target.grant_partial_language(/datum/language/common, text2num(client_source?.prefs?.read_preference(/datum/preference/choiced/csl_strength)) || 90, type)
	RegisterSignal(quirk_target, COMSIG_SPECIES_GAIN, PROC_REF(reremove_common))
	RegisterSignal(quirk_target, COMSIG_MOB_SAY, PROC_REF(translate_parts))
	native_language = get_native_language()

/datum/quirk/csl/remove()
	UnregisterSignal(quirk_target, COMSIG_SPECIES_GAIN)
	UnregisterSignal(quirk_target, COMSIG_MOB_SAY)

	if(QDELING(quirk_target))
		return

	quirk_target.remove_partial_language(/datum/language/common, type)
	...
	if(istype(carbon_quirk_holder) && carbon_quirk_holder.dna.species)
		...
		if(LAZYACCESS(species_holder.spoken_languages, /datum/language/common))
			quirk_target.grant_language(/datum/language/common, UNDERSTOOD_LANGUAGE, LANGUAGE_SPECIES)
	else
		quirk_target.grant_language(/datum/language/common, UNDERSTOOD_LANGUAGE, LANGUAGE_ATOM)
...
/datum/quirk/csl/proc/get_native_language()
	var/list/language_pool = quirk_target.get_language_holder()?.spoken_languages?.Copy()
	...
	var/obj/item/organ/tongue/tongue = quirk_target.get_organ_by_type(/obj/item/organ/tongue)
