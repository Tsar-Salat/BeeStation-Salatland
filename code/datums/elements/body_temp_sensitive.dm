...
/datum/element/body_temp_sensitive/proc/on_life(datum/target, delta_time, times_fired)
	SIGNAL_HANDLER
	...
	if(living_mob.bodytemperature < min_body_temp)
		living_mob.adjustFireLoss(cold_damage * delta_time, forced = TRUE)
		...
		living_mob.adjustFireLoss(heat_damage * delta_time, forced = TRUE)
		switch(heat_damage)
			if(1 to 5)
