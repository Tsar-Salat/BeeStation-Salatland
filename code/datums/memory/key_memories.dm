...
/// A being with a master will remember their name
/datum/memory/key/my_master
	var/masters_name

/datum/memory/key/my_master/New(
	datum/mind/memorizer_mind,
	atom/protagonist,
	atom/deuteragonist,
	atom/antagonist,
	...
	src.masters_name = masters_name
	return ..()

/datum/memory/key/my_master/get_names()
	return list("The name of my glorious master, who brought me to life, [masters_name].")
...
/datum/memory/key/nuke_code/get_starts()
	return list(
		...
		"A piece of paper with the number [nuclear_code] being handed to [protagonist_name] from a figure in a blood-red hardsuit.",
	)

/// The nukies hardsuit tracking, copy of nuke code memory essentially
/datum/memory/key/tracking_frequency
	var/team_frequency

/datum/memory/key/tracking_frequency/New(
	datum/mind/memorizer_mind,
	atom/protagonist,
	atom/deuteragonist,
	atom/antagonist,
	...
	src.team_frequency = team_frequency
	return ..()

/datum/memory/key/tracking_frequency/get_names()
	return list("[protagonist_name] learns the tracking frequency for the syndicates hardsuits, [team_frequency].")

/datum/memory/key/tracking_frequency/get_starts()
	return list(
		"The number [team_frequency] written on a sticky note with the words \"FOR SYNDICATE EYES ONLY\" scrawled next to it.",
		"A piece of paper with the number [team_frequency] being handed to [protagonist_name] from a figure in a blood-red hardsuit.",
	)
