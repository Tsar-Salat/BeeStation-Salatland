		...
		for(var/spawner_obj in GLOB.mob_spawners[spawner])
			if(!this["desc"])
				if(istype(spawner_obj, /obj/effect/mob_spawn))
					...
					var/atom/movable/object = spawner_obj
					if(isslime(object))
						this["short_desc"] = object.get_spawner_desc()
						this["flavor_text"] = object.get_spawner_flavour_text()
					else
...
/datum/spawners_menu/ui_state(mob/user, datum/tgui/ui)
	return GLOB.observer_state
...
/datum/spawners_menu/ui_act(action, params)
	...
	if(!LAZYLEN(spawnerlist))
		return
	var/atom/movable/mob_spawner = pick(spawnerlist)
