...
/mob/camera/blob/proc/create_special(price, blobstrain, min_separation, needs_node, turf/tile)
	if(!tile)
		tile = get_turf(src)
	var/obj/structure/blob/B = (locate(/obj/structure/blob) in tile)
	if(!B)
	...
	if(min_separation)
		for(var/obj/structure/blob/other_blob in orange(min_separation, tile))
			if(other_blob.type == blobstrain)
				to_chat(src, "<span class='warning'>There is a similar blob nearby, move more than [min_separation] tiles away from it!</span>")
...
/mob/camera/blob/proc/create_shield(turf/T)
	...
	if(S)
		...
		create_special(BLOB_UPGRADE_STRONG_COST, /obj/structure/blob/shield, 0, FALSE, T)
