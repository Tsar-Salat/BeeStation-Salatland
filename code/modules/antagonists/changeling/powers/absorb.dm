...
/datum/action/changeling/absorb_dna/can_sting(mob/living/carbon/owner)
	...
	if(!target || !iscarbon(target))
		to_chat(owner, "<span class='warning'>We must be grabbing a creature to absorb them!</span>")
		changeling.is_absorbing = FALSE
		return
	if(changeling.is_absorbing)
		to_chat(owner, "<span class='warning'>We are already absorbing!</span>")
		return
	if(HAS_TRAIT(target, TRAIT_NODEATH))
		to_chat(owner, "<span class='warning'>We can't absorb this creature.</span>")
		return
	if(owner.grab_state <= GRAB_NECK)
		to_chat(owner, "<span class='warning'>We must have a tighter grip to absorb this creature!</span>")
		return
...
/datum/action/changeling/absorb_dna/sting_action(mob/owner)
	...
	changeling.is_absorbing = TRUE

	if(!attempt_absorb(target))
		return

	SSblackbox.record_feedback("nested tally", "changeling_powers", 1, list("Absorb DNA", "4"))
	owner.visible_message("<span class='danger'>[owner] sucks the fluids from [target]!</span>", "<span class='notice'>We have absorbed [target].</span>")
	...
	if(target.mind && owner.mind)//if the victim and user have minds
...
/datum/action/changeling/absorb_dna/proc/absorb_memories(mob/living/carbon/human/target)
	...
	if(recent_speech.len)
		...
		to_chat(owner, "<span class='boldnotice'>Some of [target]'s speech patterns, we should study these to better impersonate [target.p_them()]!</span>")
		...
		to_chat(owner, "<span class='boldnotice'>We have no more knowledge of [target]'s speech patterns.</span>")


	var/datum/antagonist/changeling/target_ling = target.mind.has_antag_datum(/datum/antagonist/changeling)
	if(target_ling)//If the target was a changeling, suck out their extra juice and objective points!
		to_chat(owner, "<span class='boldnotice'>[target] was one of us. We have absorbed their power.</span>")
...
/datum/action/changeling/absorb_dna/proc/attempt_absorb(mob/living/carbon/human/target)
	...
	for(var/absorbing_iteration in 1 to 3)
		switch(absorbing_iteration)
			if(1)
				to_chat(owner, "<span class='notice'>This creature is compatible. We must hold still...</span>")
			if(2)
				owner.visible_message("<span class='warning'>[owner] extends a proboscis!</span>", "span class='notice'>We extend a proboscis.</span>")
			if(3)
				owner.visible_message("<span class='danger'>[owner] stabs [target] with the proboscis!</span>", "<span class='notice'>We stab [target] with the proboscis.</span>")
		...
		if(!do_after(owner, 15 SECONDS, target))
			to_chat(owner, "<span class='warning'>Our absorption of [target] has been interrupted!</span>")
			changeling.is_absorbing = FALSE
			return FALSE
	return TRUE
