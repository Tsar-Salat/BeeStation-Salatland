...
/datum/objective/assassinate/jealous/find_target(list/dupe_search_range, list/blacklist)//returning null = free objective
	...
	if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_SECURITY_NAME))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_SECURITY_NAME)
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_ENGINEERING_NAME))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_ENGINEERING_NAME)
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_MEDICAL_NAME))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_MEDICAL_NAME)
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_SCIENCE_NAME))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_SCIENCE_NAME)
	else if(obsession.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_CARGO_NAME))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_CARGO_NAME)
	else if(obsession.assigned_role in (SSdepartment.get_jobs_by_dept_id(DEPARTMENT_CIVILIAN_NAME)))
		chosen_department = SSdepartment.get_jobs_by_dept_id(DEPARTMENT_CIVILIAN_NAME)
	else
		set_target(null)
		update_explanation_text()
		return
