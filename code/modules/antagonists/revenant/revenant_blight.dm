...
/datum/disease/revblight/stage_act(delta_time, times_fired)
	...
	switch(stage)
		...
		if(5)
			if(affected_mob.stamina.loss_as_percent <= 100) //check required to prevent randomly screaming from limbs being crippled at extreme stamina
				affected_mob.stamina.adjust(-7.5 * delta_time) //No longer realistically possible to counteract with stimulants
			if(!finalstage)
				finalstage = TRUE
