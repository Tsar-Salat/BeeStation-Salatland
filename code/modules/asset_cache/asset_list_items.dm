			...
			for(var/list/design as anything in designs)
				...
				if(ispath(path, /obj/structure/window))
					if(path == /obj/structure/window)
						sprite_icon = icon(icon = 'icons/hud/radials/radial_generic.dmi', icon_state = "windowsize")
					else if(path == /obj/structure/window/reinforced)
						sprite_icon = icon(icon = 'icons/hud/radials/radial_generic.dmi', icon_state = "windowtype")
