...
/datum/job/proc/get_exp_req_amount()
	if(title in (SSdepartment.get_jobs_by_dept_id(DEPARTMENT_COMMAND_NAME) | list(JOB_NAME_AI)))
...
/proc/job_is_xp_locked(jobtitle)
	if(!CONFIG_GET(flag/use_exp_restrictions_heads) && (jobtitle in (SSdepartment.get_jobs_by_dept_id(DEPARTMENT_COMMAND_NAME) | list(JOB_NAME_AI))))
		return FALSE
	if(!CONFIG_GET(flag/use_exp_restrictions_other) && !(jobtitle in (SSdepartment.get_jobs_by_dept_id(DEPARTMENT_COMMAND_NAME) | list(JOB_NAME_AI))))
		return FALSE
	return TRUE
