...
/// Returns a list of db_keys derived from the preference typepaths in [customization_options]. Used in quirks middleware to supply the preferences to render.
/datum/quirk_constant_data/proc/get_customization_data()
	...
	for (var/datum/preference/pref_type as anything in customization_options)
		...
		customization_data += pref_instance.db_key
