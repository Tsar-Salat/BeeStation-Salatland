...
/datum/religion_sect/proc/sect_bless(mob/living/target, mob/living/chap)
	...
	var/heal_amt = 10
	var/list/hurt_limbs = blessed.get_damaged_bodyparts(1, 1, status = BODYTYPE_ORGANIC)

	if(hurt_limbs.len)
		for(var/X in hurt_limbs)
			var/obj/item/bodypart/affecting = X
			if(affecting.heal_damage(heal_amt, heal_amt, required_status = BODYTYPE_ORGANIC))
