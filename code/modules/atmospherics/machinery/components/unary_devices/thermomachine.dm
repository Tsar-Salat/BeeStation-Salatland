...
/obj/machinery/atmospherics/components/unary/thermomachine/process_atmos()
	...
	port.return_temperature() = max(((port.return_temperature() * port_capacity) + heat_amount) / port_capacity, TCMB)
...
/obj/machinery/atmospherics/components/unary/thermomachine/screwdriver_act(mob/living/user, obj/item/tool)
	if(on)
		to_chat(user, "<span class='notice'>You can't open [src] while it's on!</span>")
...
/obj/machinery/atmospherics/components/unary/thermomachine/proc/check_pipe_on_turf()
	for(var/obj/machinery/atmospherics/device in get_turf(src))
		...
		if(device.piping_layer == piping_layer)
			visible_message("<span class='warning'>A pipe is hogging the port, remove the obstruction or change the machine piping layer.</span>")
			return TRUE
	return FALSE
