...
#define GET_MOLES_GASTYPE(gas_mixture, gas_type) (gas_mixture.gases[gas_type] ? gas_mixture.gases[gas_type][MOLES] : 0)
...
/datum/unit_test/lungs/proc/lungs_test_check_breath(test_name, mob/living/carbon/human/lab_rat, obj/item/organ/lungs/test_lungs, datum/gas_mixture/test_mix, expect_failure = FALSE)
	...
	if(test_breath.total_moles() > 0)
		oxygen_pp = test_breath.get_breath_partial_pressure(GET_MOLES_GASTYPE(test_breath, /datum/gas/oxygen))
		nitro_pp = test_breath.get_breath_partial_pressure(GET_MOLES_GASTYPE(test_breath, /datum/gas/nitrogen))
		co2_pp = test_breath.get_breath_partial_pressure(GET_MOLES_GASTYPE(test_breath, /datum/gas/carbon_dioxide))
		plasma_pp = test_breath.get_breath_partial_pressure(GET_MOLES_GASTYPE(test_breath, /datum/gas/plasma))
	...
	var/expected_oxygen = GET_MOLES_GASTYPE(test_breath_backup, /datum/gas/oxygen)
	var/expected_nitro = GET_MOLES_GASTYPE(test_breath_backup, /datum/gas/nitrogen)
	var/expected_co2 = GET_MOLES_GASTYPE(test_breath_backup, /datum/gas/carbon_dioxide)
	var/expected_plasma = GET_MOLES_GASTYPE(test_breath_backup, /datum/gas/plasma)
	...
	if(min_oxygen)
		...
		expected_oxygen = 0
	if(min_nitro)
		expected_co2 += GET_MOLES_GASTYPE(test_breath_backup, /datum/gas/nitrogen)
		expected_nitro = 0
	if(min_plasma)
		expected_co2 += GET_MOLES_GASTYPE(test_breath_backup, /datum/gas/plasma)
		expected_plasma = 0

	// Validate conversion of inhaled gas to exhaled gas.
	if(min_oxygen)
		TEST_ASSERT(molar_cmp_equals(GET_MOLES_GASTYPE(test_breath, /datum/gas/oxygen), expected_oxygen), TEST_CHECK_BREATH_MESSAGE(test_lungs, "should consume all Oxygen initially present in the breath."))
		TEST_ASSERT(molar_cmp_equals(GET_MOLES_GASTYPE(test_breath, /datum/gas/carbon_dioxide), expected_co2), TEST_CHECK_BREATH_MESSAGE(test_lungs, "should convert Oxygen into an equivalent volume of CO2."))
	if(min_nitro)
		TEST_ASSERT(molar_cmp_equals(GET_MOLES_GASTYPE(test_breath, /datum/gas/nitrogen), expected_nitro), TEST_CHECK_BREATH_MESSAGE(test_lungs, "should consume all Nitrogen initially present in the breath."))
		TEST_ASSERT(molar_cmp_equals(GET_MOLES_GASTYPE(test_breath, /datum/gas/carbon_dioxide), expected_co2), TEST_CHECK_BREATH_MESSAGE(test_lungs, "should convert Nitrogen into an equivalent volume of CO2."))
	if(min_plasma)
		TEST_ASSERT(molar_cmp_equals(GET_MOLES_GASTYPE(test_breath, /datum/gas/plasma), expected_plasma), TEST_CHECK_BREATH_MESSAGE(test_lungs, "should consume all Plasma initially present in the breath."))
		TEST_ASSERT(molar_cmp_equals(GET_MOLES_GASTYPE(test_breath, /datum/gas/carbon_dioxide), expected_co2), TEST_CHECK_BREATH_MESSAGE(test_lungs, "should convert Plasma into an equivalent volume of CO2."))
