...
/obj/item/clothing/under/ethereal_tunic/worn_overlays(mutable_appearance/standing, isinhands, icon_file)
	. = ..()
	if(!isinhands)
		. += emissive_appearance('icons/mob/clothing/under/ethereal.dmi', "eth_tunic_emissive_worn", layer, alpha = src.alpha)
		ADD_LUM_SOURCE(src, LUM_SOURCE_MANAGED_OVERLAY)

/obj/item/clothing/under/ethereal_tunic/update_overlays()
	. = ..()
	. += emissive_appearance('icons/obj/clothing/under/ethereal.dmi', "eth_tunic_emissive", layer, alpha = src.alpha)
	ADD_LUM_SOURCE(src, LUM_SOURCE_MANAGED_OVERLAY)
...
/obj/item/clothing/under/ethereal_tunic/trailwarden/equipped(mob/living/user, slot)
	...
	if(isethereal(user) && slot == ITEM_SLOT_ICLOTHING)
		...
		to_chat(ethereal, "<span class='notice'>[src] gently quivers for a moment as you put it on.</span>")
		set_greyscale(ethereal.dna.species.fixed_mut_color)
		ethereal.update_inv_w_uniform()
