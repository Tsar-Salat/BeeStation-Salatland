...
/obj/structure/cable/proc/mergeConnectedNetworks(direction)
	...
	var/inverse_dir = (!direction)? 0 : dir_inverse_multiz(direction) //flip the direction, to match with the source position on its turf
...
/obj/structure/cable/proc/cut_cable_from_powernet(remove = TRUE)
	...
	for(var/dir_check in GLOB.cardinals)
		if(linked_dirs & dir_check)
			T1 = get_step_multiz(T1, dir_check)
	...
	for(var/obj/O in P_list)
		if(first)
			first = FALSE
			continue
		addtimer(CALLBACK(O, PROC_REF(auto_propogate_cut_cable), O), delay) //so we don't rebuild the network X times when singulo/explosion destroys a line of X cables
		delay += 5
