...
/obj/structure/pipe_cleaner/Destroy()					// called when a cable is deleted
	if(powernet)
...
/obj/structure/pipe_cleaner/deconstruct(disassembled = TRUE)
	if(!(flags_1 & NODECONSTRUCT_1))
		...
		if((d1 == DOWN || d2 == DOWN) && T_below)
			for(var/obj/structure/pipe_cleaner/C in T_below)
				if(C.d1 == UP || C.d2 == UP)
					C.deconstruct()
	..()
...
/obj/structure/pipe_cleaner/examine(mob/user)
	. = ..()
	if(isobserver(user))
		. += get_power_info()

// shock the user with probability prb
/obj/structure/pipe_cleaner/proc/shock(mob/user, prb, siemens_coeff = 1)
	if(!prob(prb))
		return 0
...
/obj/structure/pipe_cleaner/proc/get_power_info()
...
/obj/structure/pipe_cleaner/proc/add_avail(amount)
	if(powernet)
		powernet.newavail += amount

/obj/structure/pipe_cleaner/proc/add_load(amount)
	if(powernet)
		powernet.load += amount

/obj/structure/pipe_cleaner/proc/surplus()
	if(powernet)
		return clamp(powernet.avail-powernet.load, 0, powernet.avail)
	else
		return 0

/obj/structure/pipe_cleaner/proc/avail(amount)
	if(powernet)
		return amount ? powernet.avail >= amount : powernet.avail
	else
		return 0

/obj/structure/pipe_cleaner/proc/add_delayedload(amount)
	if(powernet)
		powernet.delayedload += amount

/obj/structure/pipe_cleaner/proc/delayed_surplus()
	if(powernet)
		return clamp(powernet.newavail - powernet.delayedload, 0, powernet.newavail)
	else
		return 0

/obj/structure/pipe_cleaner/proc/newavail()
	if(powernet)
...
/obj/structure/pipe_cleaner/proc/mergeDiagonalsNetworks(direction)
...
/obj/structure/pipe_cleaner/proc/mergeConnectedNetworks(direction)
	...
	for(var/obj/structure/pipe_cleaner/C in TB)
...
/obj/structure/pipe_cleaner/proc/mergeConnectedNetworksOnTurf()
	...
	for(var/AM in loc)
		if(istype(AM, /obj/structure/pipe_cleaner))
			var/obj/structure/pipe_cleaner/C = AM
...
/obj/structure/pipe_cleaner/proc/get_connections(powernetless_only = 0)
...
/obj/structure/pipe_cleaner/proc/denode()
	var/turf/T1 = loc
	if(!T1)
		return
...
/obj/structure/pipe_cleaner/proc/auto_propogate_cut_cable(obj/O)
	if(O && !QDELETED(O))
		var/datum/powernet/newPN = new()// creates a new powernet...
		propagate_network(O, newPN)//... and propagates it to the other side of the cable

// cut the cable's powernet at this cable and updates the powergrid
/obj/structure/pipe_cleaner/proc/cut_cable_from_powernet(remove=TRUE)
	var/turf/T1 = loc
	var/list/P_list
	if(!T1)
		return
...
/obj/item/stack/cable_coil/proc/place_turf(turf/T, mob/user, dirnew)
	...
	var/obj/structure/pipe_cleaner/C = place_cable(T, user, d1, d2)
	if(C.shock(user, 50))
		if(prob(50)) //fail
...
/obj/item/stack/cable_coil/proc/place_cable(turf/open/T, mob/user, d1, d2)
	if(!istype(T))
		return
	var/obj/structure/pipe_cleaner/C = get_new_cable(T)
...
/obj/item/stack/cable_coil/proc/cable_join(obj/structure/cable/C, mob/user, var/showerror = TRUE, forceddir)
	...
	if((C.d1 == dirn || C.d2 == dirn) && !forceddir)
		...
		if(U.underfloor_accessibility < UNDERFLOOR_INTERACTABLE) //can't place a cable if it's a plating with a tile on it
			...
			for(var/obj/structure/pipe_cleaner/LC in U) // check to make sure there's not a cable there already
				if(LC.d1 == fdirn || LC.d2 == fdirn)
					if (showerror)
						to_chat(user, "<span class='warning'>There's already a cable at that position!</span>")
					return

			var/obj/structure/pipe_cleaner/NC = get_new_cable (U)
	...
	// exisiting cable doesn't point at our position or we have a supplied direction, so see if it's a stub
		...
		for(var/obj/structure/pipe_cleaner/LC in T)		// check to make sure there's no matching cable
