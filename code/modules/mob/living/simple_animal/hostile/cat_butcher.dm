...
/mob/living/simple_animal/hostile/cat_butcherer/AttackingTarget()
	if(ishuman(target))
		...
		if(!L.getorgan(/obj/item/organ/ears/human/cat) && L.stat) //target doesnt have cat ears
			visible_message("[src] slices off [L]'s ears, and replaces them with cat ears!", span_notice("You replace [L]'s ears with cat ears'."))
			var/obj/item/organ/ears/human/cat/newears = new
			newears.Insert(L)
			...
			newtail.Insert(L)
			return
		else if(!L.has_trauma_type(/datum/brain_trauma/severe/pacifism) && L.getorgan(/obj/item/organ/ears/human/cat) && L.getorgan(/obj/item/organ/tail/human/cat) && L.stat) //still does damage.
...
/mob/living/simple_animal/hostile/cat_butcherer/CanAttack(atom/the_target)
	if(iscarbon(target))
		...
		if(C.getorgan(/obj/item/organ/ears/human/cat) && C.getorgan(/obj/item/organ/tail/human/cat) && C.has_trauma_type(/datum/brain_trauma/severe/pacifism))//he wont attack his creations
