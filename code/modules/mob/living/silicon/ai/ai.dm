	...
	for(var/mob/dead/observer/each_ghost in GLOB.dead_mob_list)
		...
		if(each_ghost.has_language(message_language))
			...
			if(!rendered_scrambled_message)
				var/ghost_translation = each_ghost.translate_language(speaker, message_language, raw_message)
				var/atom/movable/ghost_source = speaker.GetSource() || speaker
				rendered_scrambled_message = span_message(each_ghost.say_emphasis(ghost_source.say_quote(ghost_translation, spans, message_mods)))
