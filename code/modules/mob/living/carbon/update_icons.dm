...
/mob/living/carbon/proc/update_body_parts(var/update_limb_data)
	...
	for(var/obj/item/bodypart/BP as() in bodyparts)
		...
		var/old_key = icon_render_keys?[BP.body_zone] //Checks the mob's icon render key list for the bodypart
		icon_render_keys[BP.body_zone] = (BP.is_husked) ? generate_husk_key(BP) : generate_icon_key(BP) //Generates a key for the current bodypart

		if(icon_render_keys[BP.body_zone] != old_key || get_top_offset() != last_top_offset) //If the keys match, that means the limb doesn't need to be redrawn
			needs_update += BP
