...
/datum/species/proc/handle_hair(mob/living/carbon/human/H, forced_colour)
	...
	if(H.facial_hair_style && (FACEHAIR in species_traits) && (!facialhair_hidden || dynamic_fhair_suffix))
		S = SSaccessories.facial_hairstyles_list[H.facial_hair_style]
		if(S?.icon_state)
	...
	if(!hair_hidden || dynamic_hair_suffix || worn_wig)
		...
		if(!hair_hidden && !H.get_organ_slot(ORGAN_SLOT_BRAIN) && !HAS_TRAIT(H, TRAIT_NOBLOOD))
			...
			S = SSaccessories.hairstyles_list[current_hair_style]
			if(S?.icon_state)
				...
				if(!forced_colour)
					...
					if(gradient_style)
						var/datum/sprite_accessory/gradient = SSaccessories.hair_gradients_list[gradient_style]
...
/datum/species/proc/handle_body(mob/living/carbon/human/H)
	...
	if(HAS_MARKINGS in species_traits)
		...
		var/datum/sprite_accessory/markings = SSaccessories.moth_markings_list[H.dna.features["moth_markings"]]
	...
	if(!(NO_UNDERWEAR in species_traits))
		if(H.underwear && !(H.dna.species.bodytype & BODYTYPE_DIGITIGRADE))
			var/datum/sprite_accessory/underwear/underwear = SSaccessories.underwear_list[H.underwear]
		...
		if(H.undershirt)
			var/datum/sprite_accessory/undershirt/undershirt = SSaccessories.undershirt_list[H.undershirt]
			if(undershirt)
		...
		if(H.socks && H.num_legs >= 2 && !(H.dna.species.bodytype & BODYTYPE_DIGITIGRADE) && !(NOSOCKS in species_traits))
			var/datum/sprite_accessory/socks/socks = SSaccessories.socks_list[H.socks]
			if(socks)
...
/datum/species/proc/handle_mutant_bodyparts(mob/living/carbon/human/H, forced_colour)
	...
	if(mutant_bodyparts["tail_cat"])
		if(H.wear_suit && (H.wear_suit.flags_inv & HIDEJUMPSUIT))
			bodyparts_to_add -= "tail_cat"


	if(mutant_bodyparts["waggingtail_human"])
		if(H.wear_suit && (H.wear_suit.flags_inv & HIDEJUMPSUIT))
			bodyparts_to_add -= "waggingtail_human"
		else if (mutant_bodyparts["tail_cat"])
			bodyparts_to_add -= "waggingtail_human"
	...
	for(var/layer in relevent_layers)
		...
		for(var/bodypart in bodyparts_to_add)
			...
			switch(bodypart)
				if("tail_lizard")
					S = SSaccessories.tails_list_lizard[H.dna.features["tail_lizard"]]
				if("waggingtail_lizard")
					S = SSaccessories.animated_tails_list_lizard[H.dna.features["tail_lizard"]]
				if("tail_cat")
					S = SSaccessories.tails_list_human[H.dna.features["tail_cat"]]
				if("waggingtail_human")
					S = SSaccessories.animated_tails_list_human[H.dna.features["tail_cat"]]
				if("spines")
					S = SSaccessories.spines_list[H.dna.features["spines"]]
				if("waggingspines")
					S = SSaccessories.animated_spines_list[H.dna.features["spines"]]
				if("snout")
					S = SSaccessories.snouts_list[H.dna.features["snout"]]
				if("frills")
					S = SSaccessories.frills_list[H.dna.features["frills"]]
				if("horns")
					S = SSaccessories.horns_list[H.dna.features["horns"]]
				if("ears")
					S = SSaccessories.ears_list[H.dna.features["ears"]]
				if("lizard_markings")
					S = SSaccessories.lizard_markings_list[H.dna.features["lizard_markings"]]
				if("wings")
					S = SSaccessories.wings_list[H.dna.features["wings"]]
				if("wingsopen")
					S = SSaccessories.wings_open_list[H.dna.features["wings"]]
				if("legs")
					S = SSaccessories.legs_list[H.dna.features["legs"]]
				if("moth_wings")
					if(HAS_TRAIT(H, TRAIT_MOTH_BURNT))
						S = SSaccessories.moth_wings_list["Burnt Off"]
					else
						S = SSaccessories.moth_wings_list[H.dna.features["moth_wings"]]
				if("moth_antennae")
					if(HAS_TRAIT(H, TRAIT_MOTH_BURNT))
						S = SSaccessories.moth_antennae_list["Burnt Off"]
					else
						S = SSaccessories.moth_antennae_list[H.dna.features["moth_antennae"]]
				if("moth_wingsopen")
					S = SSaccessories.moth_wingsopen_list[H.dna.features["moth_wings"]]
				if("moth_markings")
					S = SSaccessories.moth_markings_list[H.dna.features["moth_markings"]]
				if("ipc_screen")
					S = SSaccessories.ipc_screens_list[H.dna.features["ipc_screen"]]
				if("ipc_antenna")
					S = SSaccessories.ipc_antennas_list[H.dna.features["ipc_antenna"]]
				if("ipc_chassis")
					S = SSaccessories.ipc_chassis_list[H.dna.features["ipc_chassis"]]
				if("insect_type")
					S = SSaccessories.insect_type_list[H.dna.features["insect_type"]]
				if("apid_antenna")
					S = SSaccessories.apid_antenna_list[H.dna.features["apid_antenna"]]
				if("apid_stripes")
					S = SSaccessories.apid_stripes_list[H.dna.features["apid_stripes"]]
				if("apid_headstripes")
					S = SSaccessories.apid_headstripes_list[H.dna.features["apid_headstripes"]]
				if("psyphoza_cap")
					S = SSaccessories.psyphoza_cap_list[H.dna.features["psyphoza_cap"]]
				if("diona_leaves")
					S = SSaccessories.diona_leaves_list[H.dna.features["diona_leaves"]]
				if("diona_thorns")
					S = SSaccessories.diona_thorns_list[H.dna.features["diona_thorns"]]
				if("diona_flowers")
					S = SSaccessories.diona_flowers_list[H.dna.features["diona_flowers"]]
				if("diona_moss")
					S = SSaccessories.diona_moss_list[H.dna.features["diona_moss"]]
				if("diona_mushroom")
					S = SSaccessories.diona_mushroom_list[H.dna.features["diona_mushroom"]]
				if("diona_antennae")
					S = SSaccessories.diona_antennae_list[H.dna.features["diona_antennae"]]
				if("diona_eyes")
					S = SSaccessories.diona_eyes_list[H.dna.features["diona_eyes"]]
				if("diona_pbody")
					S = SSaccessories.diona_pbody_list[H.dna.features["diona_pbody"]]
			...
			if(bodypart == "tail_lizard" || bodypart == "tail_cat")
				bodypart = "tail"
