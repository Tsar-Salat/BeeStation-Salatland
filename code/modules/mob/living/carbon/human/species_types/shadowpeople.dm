...
/datum/species/shadow/spec_life(mob/living/carbon/human/H, delta_time, times_fired)
	...
	if(istype(T))
		...
		if(light_amount > SHADOW_SPECIES_LIGHT_THRESHOLD) //if there's enough light, start dying
			H.take_overall_damage(brute = 0.5 * delta_time, burn = 0.5 * delta_time, required_status = BODYTYPE_ORGANIC)
		else if (light_amount < SHADOW_SPECIES_LIGHT_THRESHOLD) //heal in the dark
			H.heal_overall_damage(brute = 0.5 * delta_time, burn = 0.5 * delta_time, required_status = BODYTYPE_ORGANIC)
