...
/mob/living/proc/set_playable(ban_type = null, poll_ignore_key = null)
	...
	if (!key)	//check if there is nobody already inhibiting this mob
		notify_ghosts("[name] can be controlled", null, enter_link="<a href=?src=[REF(src)];activate=1>(Click to play)</a>", source=src, action=NOTIFY_PLAY, ignore_key = poll_ignore_key)
