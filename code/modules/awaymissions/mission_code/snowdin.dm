...
/turf/open/lava/plasma/do_burn(atom/movable/burn_target, delta_time = 1)
	...
	if(istype(burn_species, /datum/species/plasmaman) || istype(burn_species, /datum/species/android) || istype(burn_species, /datum/species/ipc)) //ignore plasmamen/robotic species
		return
	...
	if(plasma_parts.len)
		...
		burn_human.visible_message("<span class='warning'>[burn_human] screams in pain as [burn_human.p_their()] [burn_limb] melts down to the bone!</span>", \
			"<span class='userdanger'>You scream out in pain as your [burn_limb] melts down to the bone, leaving an eerie plasma-like glow where flesh used to be!</span>")
	if(!plasma_parts.len && !robo_parts.len) //a person with no potential organic limbs left AND no robotic limbs, time to turn them into a plasmaman
		burn_human.IgniteMob()
		burn_human.set_species(/datum/species/plasmaman)
		burn_human.visible_message("<span class='warning'>[burn_human] bursts into a brilliant purple flame as [burn_human.p_their()] entire body is that of a skeleton!</span>", \
