...
/obj/vehicle/sealed/mecha/working/proc/collect_ore()
	if(!box)
		return
	if(!(locate(/obj/item/mecha_parts/mecha_equipment/hydraulic_clamp) in flat_equipment) /*&& !(locate(/obj/item/mecha_parts/mecha_equipment/orebox_manager) in equipment)*/)
		return
