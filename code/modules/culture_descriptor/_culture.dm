...
/datum/cultural_info/proc/get_random_name(gender)
...
/datum/cultural_info/proc/sanitize_name(new_name)
	return reject_bad_name(new_name)

#define COLLAPSED_CULTURE_BLURB_LEN 48
/datum/cultural_info/proc/get_description(header, append, verbose = TRUE)
	var/list/dat = list()
...
/datum/cultural_info/proc/get_text_body()
	return description

/datum/cultural_info/proc/get_text_details()
	. = list()
...
/datum/cultural_info/proc/get_spoken_languages()
	. = list()
	if(language)                  . |= language
	if(default_language)          . |= default_language
	if(LAZYLEN(additional_langs)) . |= additional_langs

/datum/cultural_info/proc/get_formal_name_suffix()
	return

/datum/cultural_info/proc/get_formal_name_prefix()
	return

/datum/cultural_info/proc/get_qualifications()
	return qualifications

/datum/cultural_info/proc/get_possible_personal_goals(department_flag)
	return
