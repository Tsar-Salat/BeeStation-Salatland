/obj/item/bodypart/head/lizard
	icon = 'icons/mob/human/species/lizard/bodyparts.dmi'
...
/obj/item/bodypart/l_leg/digitigrade/update_limb(dropping_limb = FALSE, source, is_creating = FALSE)
	. = ..()
	if(ishuman(owner))
...
/obj/item/bodypart/r_leg/digitigrade/update_limb(dropping_limb = FALSE, source, is_creating = FALSE)
	. = ..()
	if(ishuman(owner))
		var/mob/living/carbon/human/human_owner = owner
		var/obj/item/clothing/shoes/worn_shoes = human_owner.get_item_by_slot(ITEM_SLOT_FEET)
