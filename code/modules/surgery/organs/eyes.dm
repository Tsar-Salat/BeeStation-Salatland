...
/obj/item/organ/eyes/proc/generate_body_overlay(mob/living/carbon/human/parent)
	...
	var/mutable_appearance/eye_left = mutable_appearance(eye_icon, "[eye_icon_state]_l", CALCULATE_MOB_OVERLAY_LAYER(EYES_LAYER), parent)
	var/mutable_appearance/eye_right = mutable_appearance(eye_icon, "[eye_icon_state]_r", CALCULATE_MOB_OVERLAY_LAYER(EYES_LAYER), parent)
	var/list/overlays = list(eye_left, eye_right)

	var/obscured = parent.check_obscured_slots(TRUE)
	if(overlay_ignore_lighting && !(obscured & ITEM_SLOT_EYES))
		overlays += emissive_appearance(eye_left.icon, eye_left.icon_state, layer = CALCULATE_MOB_OVERLAY_LAYER(EYES_LAYER), alpha = eye_left.alpha)
		overlays += emissive_appearance(eye_right.icon, eye_right.icon_state, layer = CALCULATE_MOB_OVERLAY_LAYER(EYES_LAYER), alpha = eye_right.alpha)
...
/obj/item/organ/eyes/proc/setup_eyelids(mutable_appearance/eye_left, mutable_appearance/eye_right, mob/living/carbon/human/parent)
	...
	var/mutable_appearance/left_eyelid_overlay = mutable_appearance(layer = CALCULATE_MOB_OVERLAY_LAYER(EYES_LAYER))
	var/mutable_appearance/right_eyelid_overlay = mutable_appearance(layer = CALCULATE_MOB_OVERLAY_LAYER(EYES_LAYER))
