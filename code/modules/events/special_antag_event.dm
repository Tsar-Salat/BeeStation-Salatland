...
/datum/round_event/create_special_antag/start()
	for(var/mob/living/carbon/human/H in shuffle(GLOB.player_list))
		...
		if(H.stat == DEAD)
			continue
		if(!SSjob.GetJob(H.mind.assigned_role) || (H.mind.assigned_role in SSdepartment.get_jobs_by_dept_id(DEPARTMENT_SILICON_NAME))) //only station jobs sans nonhuman roles, prevents ashwalkers trying to stalk with crewmembers they never met
			continue
