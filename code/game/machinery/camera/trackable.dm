...
/datum/trackable/proc/set_tracked_mob(mob/living/track)
	...
	if(tracked_mob)
		UnregisterSignal(tracked_mob, list(COMSIG_PARENT_QDELETING, COMSIG_MOVABLE_MOVED,/* COMSIG_MOVABLE_UPDATE_GLIDE_SIZE*/))
	if(track && !isliving(track))
		tracked_mob = null
		return
	tracked_mob = track
	if(tracked_mob)
		RegisterSignal(tracked_mob, COMSIG_PARENT_QDELETING, PROC_REF(target_deleted))
...
/datum/trackable/process()
	...
	if(cameraticks < CAMERA_TICK_LIMIT)
		if(!cameraticks)
			to_chat(tracking_holder, "<span class='warning'>Target is not near any active cameras. Attempting to reacquire...</span>")
		cameraticks++
		return

	to_chat(tracking_holder, "<span class='warning'>Unable to reacquire, cancelling track...</span>")
	reset_tracking()
...
/datum/trackable/proc/track_input(mob/living/tracker)
	...
	if(isnull(mob_ref))
		to_chat(tracker, "<span class='notice'>Target is not on or near any active cameras. Tracking failed.</span>")
		return
...
/datum/trackable/proc/track_name(mob/living/tracker, tracked_mob_name)
	...
	if(isnull(mob_ref))
		to_chat(tracker, "<span class='notice'>Target is not on or near any active cameras. Tracking failed.</span>")
		return
	to_chat(tracker, "<span class='notice'>Now tracking [tracked_mob_name] on camera.</span>")
	set_tracked_mob(mob_ref.resolve())
