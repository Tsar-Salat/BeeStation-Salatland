			...
			if(R)
				for(var/each in available_paycheck_departments)
					if(!(SSeconomy.get_budget_acc_bitflag(each) & region_access_payment))
						continue
					if(R.active_dept & SSeconomy.get_budget_acc_bitflag(each))
...
/obj/machinery/computer/card/Topic(href, href_list)
	...
	switch(href_list["choice"])
				...
				if(t1 == "Custom")
					...
					if(R)
						for(var/each in B.payment_per_department)
							if(SSeconomy.is_nonstation_account(each)) // do not touch VIP/Command flag
								continue
							R.active_dept &= ~SSeconomy.get_budget_acc_bitflag(each) // turn off all bitflag for each department except for VIP/Command. *note: this actually shouldn't use `get_budget_acc_bitflag()` proc, because bitflags are the same but these have a different purpose.
						R.active_dept &= ~DEPT_BITFLAG_COM  // micromanagement2. the reason is the same. Command should be removed manually.
					...
					if(R && jobdatum) // 1-B: reseting crew manifest
						for(var/each in available_paycheck_departments)
							if(SSeconomy.is_nonstation_account(each))
								continue
							R.active_dept &= ~SSeconomy.get_budget_acc_bitflag(each)
						R.active_dept &= ~DEPT_BITFLAG_COM  // micromanagement2
					...
					if(R && jobdatum) // 2-B: setting crew manifest
						R.active_dept |= jobdatum.departments
		...
		if ("turn_on_off_department_manifest")
			...
			if(R.active_dept & target_bitflag)
				R.active_dept &= ~target_bitflag // turn off
			else
				R.active_dept |= target_bitflag // turn on
