...
/obj/item/dice/d20/fate/proc/effect(mob/living/carbon/human/user,roll)
	...
	switch(roll)
		...
		if(3)
			//Swarm of creatures
			selected_turf.visible_message(span_userdanger("A swarm of creatures surround [user]!"))
		...
		if(8)
			...
			explosion(get_turf(user), devastation_range = -1, light_impact_range = 2, flame_range = 2, magic = TRUE, explosion_cause = src)
		if(9)
			//Cold
