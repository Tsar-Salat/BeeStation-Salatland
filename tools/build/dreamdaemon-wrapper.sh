#!/bin/bash
# Generic BYOND DreamDaemon wrapper
# Auto-detects BYOND installation and runs dreamdaemon.exe appropriately for the platform
...
DREAMDAEMON_EXECUTABLE="$(get_dreamdaemon_executable)"
PLATFORM="$(get_platform)"

case "$PLATFORM" in
    "windows")
        # On Windows, run dreamdaemon.exe directly
        exec "$DREAMDAEMON_EXECUTABLE" "$@"
        ;;
    "linux")
        ...
        exec "$WINE_CMD" "$DREAMDAEMON_EXECUTABLE" "$@"
        ;;
    *)
